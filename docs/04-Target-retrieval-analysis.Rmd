...
`TCMDATA` includes a demo `DESeq2` result (Love et al., 2014) from GSE142025 (early DN vs. control) for illustration. Further details about the dataset can be found in the [original GEO page](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE142025).
...
```{r venn-3set, fig.width=8, fig.height=7, fig.align='center', out.width='80%', collapse=FALSE}
venn1 <- ggvenn_plot(venn_df)

venn2 <- ggvenn_plot(venn_df, set.color = c("#FF8748", "#5BAA56", "#B8BB5B"), stroke.color = "white")

aplot::plot_list(venn1, venn2, ncol = 2)
```
...
```{r upset-plot, fig.width=8, fig.height=5, fig.align='center', out.width='90%', collapse=FALSE}
library(aplotExtra)
...
upset_plot(gene_list)
```

If a fourth set (e.g., CTD targets) is available, simply extend the list:

```{r upset-4set, eval=FALSE, fig.width=13, fig.height=6, fig.align='center', out.width='90%', collapse=FALSE}
gene_list4 <- list(
  DEGs        = degs,
  GeneCards   = dn_gcds,
  OpenTargets = dn_otp,
